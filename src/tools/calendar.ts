...
export function registerCalendarTools(server: McpServer, clients: UserClients) {
  ...
  server.registerTool(
    ...
    guard(async ({ account, calendarId, eventId, summary, description, location, start, end, attendees, sendUpdates, timeZone }) => {
      ...
      if (attendees !== undefined) ev.attendees = attendees.map((email) => ({ email }));
  ...
  server.registerTool(
    ...
    guard(async ({ account, calendarId, eventId, response, comment, sendUpdates }) => {
      ...
      if (!attendee) return fail("You are not listed as an attendee of this event.");
